...
# --- 1. CONFIGURATIE ---
st.set_page_config(page_title="Registratie Dienst Grondzaken Wanica Centrum",
layout="wide")
...
def stuur_mail(ontvanger, onderwerp, inhoud, bestanden=None):
 ...
 <div style="background-color: #2e7d32; padding: 20px; color: white; textalign: center;">
...
if st.session_state.logged_in:
 menu_options += [" Dossierbeheer", " Rapportages", " Agenda", "
Systeembeheer"]
...
({st.session_state.role})")
 ...
 st.session_state.update({'logged_in': True, 'role':
str(user['rol']).lower(), 'user': u_sel})
 st.rerun()
# --- 5. REGISTRATIE PAGINA ---
if menu == " Nieuwe Registratie":
 col_l, col_r = st.columns([1, 4])
 with col_l:
 st.image("https://raw.githubusercontent.com/bhikhienadeem-art/dgw-app/main/
orgineel%20logo%20Centrum.png", width=120)
 with col_r:
 ...
 tijden = [f"{h:02d}:{m:02d}" for h in range(8, 15) for m in (0, 15, 30, 45) if
not (h == 14 and m > 30)]
 cols = st.columns(6)
 for idx, t in enumerate(tijden):
 with cols[idx % 6]:
 if st.button(t, key=f"t_{t}", type="primary" if
st.session_state.selected_time == t else "secondary", use_container_width=True):
 st.session_state.selected_time = t
 st.rerun()
 else:
 st.warning("Bezoekafspraken zijn enkel op maandag en woensdag van 08:00 tot
14:30.")
 if st.button(" Registratie Indienen", type="primary", use_container_width=True):
 if all([vnaam, anaam, adres, email, id_nr, bericht]) and
st.session_state.selected_time:
 data = {"voornaam": vnaam, "achternaam": anaam, "woonadres": adres,
"email": email, "id_nummer": id_nr, "telefoon": tel, "lad_nummer": lad,
"afspraak_datum": str(datum), "afspraak_tijd": st.session_state.selected_time,
"status": "In behandeling", "bericht": bericht}
 supabase.table("aanvragen").insert(data).execute()
 mail_body = f"<b>Nieuwe aanvraag:</b> {vnaam} {anaam}<br><b>ID-nr:</b>
{id_nr}<br><b>Bericht:</b> {bericht}<br><br><b>Afspraak:</b> {datum} om
{st.session_state.selected_time}"
...
hide_index=True)
 ...
 with ce1:
 n_status = st.selectbox("Nieuwe Status", ["In behandeling", "Wacht op
documenten", "Bevestigd", "Afgehandeld"])
 n_datum = st.date_input("Verzet Datum",
value=datetime.datetime.strptime(d['afspraak_datum'], '%Y-%m-%d').date())
 n_tijd = st.text_input("Verzet Tijd", value=d['afspraak_tijd'])
 with ce2:
 toelichting = st.text_area("Interne Notitie",
value=d.get('medewerker_toelichting', ""))
 ...
 supabase.table("aanvragen").update({"status": n_status,
"afspraak_datum": str(n_datum), "afspraak_tijd": n_tijd, "medewerker_toelichting":
toelichting, "volgende_stappen": mail_tekst}).eq("id", sel_id).execute()
...
naar: {n_status}.\\nAfspraak: {n_datum} om {n_tijd}.\\n\\n{mail_tekst}"
 ...
 st.rerun()
 with btn_c2:
 if st.button(f" Verwijder Dossier #{sel_id}", type="secondary",
...
# --- 7. RAPPORTAGES & AGENDA ---
 ...
 res = supabase.table("aanvragen").select("voornaam, achternaam, afspraak_datum,
afspraak_tijd, status").execute()
 if res.data:
...
# --- 8. SYSTEEMBEHEER ---
 ...
 supabase.table("medewerkers").insert({"gebruikersnaam": new_user,
"wachtwoord": new_pass, "rol": new_role}).execute()
 ...
 st.error(f"Toegang geweigerd. Uw huidige rol is '{st.session_state.role}', maar
'admin' is vereist.")
