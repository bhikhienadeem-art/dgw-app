...
st.markdown("""
    ...
    /* Input velden (Wit met zwarte tekst) */
...
if menu == "📝 Nieuwe Registratie":
    ...
    if datum.weekday() in [0, 2]:
        ...
        st.warning("Enkel op maandag en woensdag.")

    if st.button("✅ INDIENEN"):
        if all([vnaam, anaam, email, id_nr, bericht]) and st.session_state.selected_time:
            data = {"voornaam": vnaam, "achternaam": anaam, "woonadres": adres, "email": email, "id_nummer": id_nr, "telefoon": tel, "afspraak_datum": str(datum), "afspraak_tijd": st.session_state.selected_time, "status": "In behandeling", "bericht": bericht}
            supabase.table("aanvragen").insert(data).execute()
            stuur_mail(EMAIL_USER, f"Nieuwe Klacht: {vnaam}", f"Registratie ontvangen van {vnaam} {anaam}.")
            st.success("Verzonden!")
            st.session_state.selected_time = None
        else:
            st.error("Vul alles in.")

# --- 7. DOSSIERBEHEER (HERSTELD) ---
    ...
    if res.data:
        ...
        n_status = st.selectbox("Status", ["In behandeling", "Bevestigd", "Afgehandeld"], index=0)
...
# --- 8. RAPPORTAGES ---
    ...
    if res.data:
        ...
        with col1:
            fig1 = px.pie(df, names='status', title="Status Verdeling", color_discrete_sequence=['#2e7d32', '#81c784', '#d32f2f'])
            st.plotly_chart(fig1, use_container_width=True)
        with col2:
            df['datum'] = pd.to_datetime(df['created_at']).dt.date
            fig2 = px.bar(df.groupby('datum').size().reset_index(name='aantal'), x='datum', y='aantal', title="Aanvragen Trend")
            st.plotly_chart(fig2, use_container_width=True)

# --- 9. SYSTEEMBEHEER (HERSTELD NAAR ORIGINEEL) ---
